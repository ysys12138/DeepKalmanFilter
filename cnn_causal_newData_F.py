...
class MyDataset(Dataset):
    ...
    def __init__(self):
        super().__init__()
        self.res_data = np.load("./data/npydata/dataset/SDK002res_data_real.npy")
        self.force_data = np.load("./data/npydata/dataset/SDK002force_data_real.npy")
        self.piezo_data = np.load("./data/npydata/dataset/SDK002filter+piezo_data_real.npy")
...
class MyValset(Dataset):
    ...
    def __init__(self):
        super().__init__()
        self.res_data = np.load("./data/npydata/dataset/SDK002res_data_val.npy")-0.008
        self.force_data = np.load("./data/npydata/dataset/SDK002force_data_val.npy")
        self.piezo_data = np.load("./data/npydata/dataset/SDK002filter+piezo_data_val.npy")
        self.x1 = torch.from_numpy(self.res_data)
        self.x2 = torch.from_numpy(self.piezo_data)
        self.y = torch.from_numpy(self.force_data)
        # self.y = torch.flatten(self.y, start_dim=1, end_dim=2)
...
def val_long(model, epoch_i):
    path = './data/npydata/elek/SDK002/'
    f_ture_long = np.load(path + 'Int+syn+force+S1_SDK_002_allinone.npy')
    vs_vd = offset(np.load(path + 'Int+syn+flt+S1_SDK_002_allinone.npy'), 0.0075)
...
if __name__ == '__main__':
    ...
    sct = torch.optim.lr_scheduler.StepLR(optimizer, 25, gamma=0.1, last_epoch=-1)
