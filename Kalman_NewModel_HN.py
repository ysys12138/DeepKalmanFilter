...
class KalmanNetwork(nn.Module):
    def __init__(self, size):
        ...
        nn.init.constant_(self.hlayerout.weight, 0)
    ...
    def forward(self, batch_size, x_init, vs, vd):
        ...
        for i in range(1, length):
            ...
            hiden_k = self.relu(self.linear(l_n))
            kn = self.linear2(hiden_k)
            hiden_theta = self.relu(self.linear3(l_n))
            theta1 = self.linear4(hiden_theta)

            hidden_h = self.relu(self.hlayer(combined_lh))
            theta_h = self.prelu(self.hlayerout(hidden_h))
        ...
        return force_predict, l_n_out, h_n_out  # returns result without extra dimension
...
def train(model, batch_size, dataloader, warmup=False):
    ...
    if warmup:
        with torch.no_grad():
            ...
            _, l_all, h_all = model(1, xn_1_zeros, vs_all, vd_all)
    ...
    for start, vs, vd, f_true in dataloader:
        if warmup:
            ...
            for s in start_points:
                ...
                index += 1
        # Train steps
        f_predict, _, _ = model(batch_size, x_init, vs, vd)
        ...
        optimizer.step()
        print('batch ', start, 'loss=', loss.item())
...
def val(model, epoch):
    ...
    f_predict, _, h = model(1, x_init, vs_tensor, vd_tensor)
...
if __name__ == '__main__':
    ...
    lr = torch.optim.lr_scheduler.StepLR(optimizer, 20, gamma=0.1, last_epoch=-1)
