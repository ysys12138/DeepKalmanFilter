...
def val(model, epoch):
    ...
    f_true, vs_numpy, vd_numpy = load_data_val(-0.0005)
    f_true = f_true[13000:21000]
    vs_numpy = vs_numpy[13000:21000]
    vd_numpy = vd_numpy[13000:21000]
    ...
    np.save("fp_numpy_KN_D", f_p)
    np.save("f_numpy_KN_D", f_true)
    ...
    plt.plot(f_true)
    plt.plot(h[0]*50)
    plt.legend(['Prediction', 'True force(N)', 'State h'])
    plt.text(x=4,
             y=4,
             s='loss=' + str(loss_val))
